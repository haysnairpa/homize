...
<div style="text-align: center; margin-bottom: 30px;">
    ...
    <h1 style="color: #30a0e0; margin: 0; font-size: 24px; font-weight: 700;">Pesanan Baru Diterima!</h1>
</div>

<div style="background-color: #f0f9ff; border-radius: 8px; padding: 20px; margin-bottom: 25px; border-left: 4px solid #30a0e0;">
    <p style="margin-top: 0; font-size: 16px;">Halo <strong>{{ $booking->merchant->nama_usaha }}</strong>,</p>
    <p style="margin-top: 0; font-size: 16px;">Anda menerima pesanan baru dengan detail sebagai berikut:</p>
</div>
...
<table style="width: 100%; border-collapse: separate; border-spacing: 0; margin-bottom: 25px; border-radius: 8px; overflow: hidden; border: 1px solid #E5E7EB;">
    ...
    <tr style="background-color: #f0f9ff;">
        <td style="padding: 12px 15px; border-bottom: 1px solid #E5E7EB; font-weight: 600;">Pelanggan</td>
        <td style="padding: 12px 15px; border-bottom: 1px solid #E5E7EB;">{{ $booking->user->nama }}</td>
    </tr>
    <tr style="background-color: white;">
    ...
    <tr style="background-color: #f0f9ff;">
        ...
        <td style="padding: 12px 15px; border-bottom: 1px solid #E5E7EB;">{{ \Carbon\Carbon::parse($booking->booking_schedule->waktu_mulai)->format('H:i') }} - {{ \Carbon\Carbon::parse($booking->booking_schedule->waktu_selesai)->format('H:i') }}</td>
    </tr>
    <tr style="background-color: white;">
        <td style="padding: 12px 15px; border-bottom: 1px solid #E5E7EB; font-weight: 600;">Alamat</td>
        <td style="padding: 12px 15px; border-bottom: 1px solid #E5E7EB;">{{ $booking->alamat_pembeli }}</td>
    </tr>
    @if($booking->catatan)
    <tr style="background-color: #f0f9ff;">
        <td style="padding: 12px 15px; border-bottom: 1px solid #E5E7EB; font-weight: 600;">Catatan</td>
        <td style="padding: 12px 15px; border-bottom: 1px solid #E5E7EB;">{{ $booking->catatan }}</td>
    </tr>
    @endif
    <tr style="background-color: white;">
        <td style="padding: 12px 15px; font-weight: 600;">Total Pembayaran</td>
        <td style="padding: 12px 15px; font-weight: 700; color: #30a0e0;">Rp {{ number_format($booking->pembayaran->amount, 0, ',', '.') }}</td>
    </tr>
</table>

<div style="text-align: center; margin-bottom: 25px;">
    @component('mail::button', ['url' => route('merchant.orders'), 'color' => 'primary'])
...
<div style="background-color: #fff8e6; border-radius: 8px; padding: 15px; margin-bottom: 25px; text-align: center; border-left: 4px solid #ffb655;">
    <p style="margin: 0; font-size: 14px; color: #996b33;">Silakan segera konfirmasi pesanan ini melalui dashboard merchant Anda.</p>
</div>
