<x-app-layout>
    ...
    <div class="py-12 bg-homize-gray">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                <div class="mb-6">
                    <a href="{{ route('transactions') }}" class="inline-flex items-center text-homize-blue hover:underline">
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                            <div>
                                ...
                                <p class="font-medium">{{ \Carbon\Carbon::parse($transaction->tanggal_booking)->format('d M Y') }}</p>
                            </div>
                            <div>
                                <span class="text-gray-600">Waktu Mulai:</span>
                                <p class="font-medium">{{ optional($transaction->waktu_mulai)->format('H:i') ?? '-' }}</p>
                            </div>
                            <div>
                                <span class="text-gray-600">Waktu Selesai:</span>
                                <p class="font-medium">{{ optional($transaction->waktu_selesai)->format('H:i') ?? '-' }}</p>
                            </div>
                            <div>
                ...
                <div class="mt-8 border-t pt-6">
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-xl font-bold text-homize-blue">Rp {{ number_format($transaction->amount, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
    ...
    <script>
        function initMap() {
            const location = { lat: {{ $transaction->latitude }}, lng: {{ $transaction->longitude }} };
