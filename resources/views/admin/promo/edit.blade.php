<x-admin-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.promo.update', $promo->id) }}" id="promo-form">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="tipe_diskon" 
                                    ...
                                    <option value="percentage" {{ old('tipe_diskon', $promo->tipe_diskon) === 'percentage' ? 'selected' : '' }}>Persentase (%)</option>
                                    <option value="fixed" {{ old('tipe_diskon', $promo->tipe_diskon) === 'fixed' ? 'selected' : '' }}>Nominal Tetap (Rp)</option>
                                </select>
                                @error('tipe_diskon')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="number" 
                                       ...
                                       value="{{ old('batas_penggunaan_global', $promo->batas_penggunaan_global) }}"
                            ...
                            <div>
                                ...
                                <input type="number" 
                                       ...
                                       value="{{ old('batas_penggunaan_per_user', $promo->batas_penggunaan_per_user) }}"
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" 
                                           ...
                                           id="maksimum_diskon" 
                                           value="{{ old('maksimum_diskon', $promo->maksimum_diskon) }}"
                        ...
                        <div class="mt-6 p-4 bg-gray-50 rounded-lg">
                            <h3 class="text-lg font-medium text-gray-900 mb-2">Statistik Penggunaan</h3>
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                                <div>
                                    <span class="text-gray-600">Total Digunakan:</span>
                                    <span class="font-semibold ml-2">{{ $promo->penggunaan->count() }} kali</span>
                                </div>
                                <div>
                                    <span class="text-gray-600">Pengguna Unik:</span>
                                    <span class="font-semibold ml-2">{{ $promo->penggunaan->unique('user_id')->count() }} user</span>
                                </div>
                                <div>
                                    <span class="text-gray-600">Total Diskon Diberikan:</span>
                                    <span class="font-semibold ml-2">Rp {{ number_format($promo->penggunaan->sum('diskon_amount') ?? 0, 0, ',', '.') }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-6">
                            ...
                            <textarea name="deskripsi" 
                                      ...
                                      placeholder="Deskripsi promo (opsional)">{{ old('deskripsi', $promo->deskripsi) }}</textarea>
                            @error('deskripsi')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <select name="target_type" 
                                        ...
                                        <option value="all" {{ old('target_type', $promo->target_type) === 'all' ? 'selected' : '' }}>Semua Layanan</option>
                                        <option value="category" {{ old('target_type', $promo->target_type) === 'category' ? 'selected' : '' }}>Kategori Tertentu</option>
                                        <option value="service" {{ old('target_type', $promo->target_type) === 'service' ? 'selected' : '' }}>Layanan Tertentu</option>
                                    </select>
                                    @error('target_type')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <!-- Category Selection -->
                                <div id="category-selection" class="{{ old('target_type', $promo->target_type) === 'category' ? '' : 'hidden' }}">
                                    ...
                                    <select name="target_kategori_id" 
                                        ...
                                        @foreach($categories as $category)
                                            <option value="{{ $category->id }}" {{ old('target_kategori_id', ($promo->target_type === 'category' ? $promo->target_id : null)) == $category->id ? 'selected' : '' }}>
                                                {{ $category->nama }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <!-- Service Selection -->
                                <div id="service-selection" class="{{ old('target_type', $promo->target_type) === 'service' ? '' : 'hidden' }}">
                                    ...
                                    <select name="target_layanan_id" 
                                        ...
                                        @foreach($services as $service)
                                            <option value="{{ $service->id }}" {{ old('target_layanan_id', $promo->target_id) == $service->id && $promo->target_type === 'service' ? 'selected' : '' }}>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="flex items-center">
                                    <input type="checkbox" 
                                           ...
                                           {{ old('is_exclusive', $promo->is_exclusive) ? 'checked' : '' }}>
                                ...
                                <div class="flex items-center">
                                    <input type="checkbox" 
                                           ...
                                           value="1"
                                           class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                                           {{ old('status_aktif', $promo->status_aktif) ? 'checked' : '' }}>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (targetKategoriSelect.value && targetTypeSelect.value === 'service') {
                targetKategoriSelect.dispatchEvent(new Event('change'));
            }
        });
    </script>
</x-admin-layout>